#!/usr/bin/env python3
"""Score the NGLY1 ten-mutation pool with one local Ridgey checkpoint.
...
def compact(record: dict) -> dict:
    ...
    return {
        ...
        "ec_3.5.1.52": probability(prediction["ec_number"], TARGET_EC),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fasta",
        default=str(ROOT / "work/ngly1_10mut_pool.fasta"),
...
def main() -> None:
    args = parse_args()
    wt = read_fasta(ROOT / "ngly1.fasta")[0][1]
    designs = read_fasta(Path(args.fasta))
    source = prepare_structure(
        content=(ROOT / "ngly1_afdb.cif").read_text(),
        filename="ngly1_afdb.cif",
        chain_id="A",
        name="WT",
    )
