...
def main() -> None:
    args = parse_args()
    wt = "".join(line.strip() for line in (ROOT / "ngly1.fasta").read_text().splitlines() if not line.startswith(">"))
    sequences = read_fasta(OUT / "ngly1_48_10mut_designs.fasta")
    final_rows = list(csv.DictReader((OUT / "ngly1_48_10mut_designs.csv").open()))
    final_by_design = {row["design"]: row for row in final_rows}
    plate_rows = list(csv.DictReader((OUT / "ngly1_48well_plate_map.csv").open()))
    plate_by_design = {row["design"]: row for row in plate_rows}
    ...
    ridgey_models = [json.loads((ROOT / "work/ridgey_local_ensemble_ec_pool10" / f"{name}.json").read_text()) for name in MODEL_NAMES]
    ...
    parent_plddt = 0.9342294931411743
    parent_ptm = 0.5973632335662842
    ...
    for design in sorted(sequences, key=lambda name: int(name.rsplit("_", 1)[1])):
        ...
        ec_probability = statistics.mean(record["ec_3.5.1.52"] for record in ridgey_records)
        parent_ec_probability = statistics.mean(record["ec_3.5.1.52"] for record in ridgey_wt)
        ...
        zinc_probability = statistics.mean(record["zinc"] for record in ridgey_records)
        parent_zinc_probability = statistics.mean(record["zinc"] for record in ridgey_wt)
        ...
        measurements = {
            ...
            "ec_3_5_1_52": {"value": ec_probability, "parent": parent_ec_probability, "delta": ec_probability - parent_ec_probability, "percent_change": pct_change(ec_probability, parent_ec_probability), "unit": "probability", "good_direction": "higher"},
            ...
            "zinc": {"value": zinc_probability, "parent": parent_zinc_probability, "delta": zinc_probability - parent_zinc_probability, "percent_change": pct_change(zinc_probability, parent_zinc_probability), "unit": "probability", "good_direction": "higher"},
            ...
            "core_ca_rmsd": {"value": float(final["core_ca_rmsd_angstrom"]), "parent": 0.0, "delta": float(final["core_ca_rmsd_angstrom"]), "percent_change": None, "unit": "angstrom", "good_direction": "lower"},
        ...
        flat_rows.append({
            ...
            "zinc_probability": zinc_probability,
            "zinc_percent_change": pct_change(zinc_probability, parent_zinc_probability),
            "plddt": plddt,
            "plddt_percent_change": pct_change(plddt, parent_plddt),
            "ptm": ptm,
            "ptm_percent_change": pct_change(ptm, parent_ptm),
            "core_ca_rmsd_angstrom": float(final["core_ca_rmsd_angstrom"]),
            "sequence": sequence,
        })
    ...
    summary = {
        ...
        "structure_warning_candidates": sum(row["structure_fit_flag_count"] > 0 for row in candidate_rows),
        "msa_warning_or_watch_candidates": sum(row["msa_flag_count"] > 0 for row in candidate_rows),
    ...
    payload = {
        "project": "NGLY1 Candidates",
        "schema_version": "1.0.0",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "provenance": {
            "repository": "https://github.com/sutterhill/ngly1-candidates",
            "analysis_commit": args.analysis_commit,
            "analysis_commit_url": f"https://github.com/sutterhill/ngly1-candidates/commit/{args.analysis_commit}",
        },
        "methods": {
            ...
            "msa": "5,000 MMseqs sequences",
            "potts": "rank-32 low-rank Potts, L2 selected lambda=1.0, n_eff=677.29",
        ...
        "parent": {
            "name": "NGLY1 WT (Q96IV0)",
            "sequence": wt,
            "length": len(wt),
            "catalytic_triad": [309, 336, 353],
            ...
            "metrics": {
                ...
                "ec_3_5_1_52": statistics.mean(record["ec_3.5.1.52"] for record in ridgey_wt),
                "is_enzyme": statistics.mean(record["is_enzyme"] for record in ridgey_wt),
                "zinc": statistics.mean(record["zinc"] for record in ridgey_wt),
                "plddt": parent_plddt,
                "ptm": parent_ptm,
    ...
    json_path = OUT / "ngly1_candidate_measurements.json"
    csv_path = OUT / "ngly1_candidate_measurements.csv"
